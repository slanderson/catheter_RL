...
from robot.devices.position_sensor import atc3dg_functions_3 as api
...
import numpy as np
from robot.devices.position_sensor.udp_connection import UDPConnection
...
def get_attached_sensors(data_dic):
    ...
    return filter(lambda x: x in data_dic, [1,2,3,4])
...
def copy_data_dict(old):
    ...
    for s in filter(lambda x: x in new, [1,2,3,4]):
        new[s] = np.copy(old[s])
    return new
...
class TrakSTARInterface(object):
    ...
    def open_data_file(self, filename, directory="data", suffix=".csv",
                       ...
                       write_udp=True, comment_line="", overwriteFile = False):
        ...
        while True:
            ...
            if os.path.isfile(directory + os.path.sep + self.filename) and not overwriteFile:
                #
    ...
    def get_synchronous_data_dict(self, write_data_file=True):
        ...
        return d
    # In reality this is get_asynchronous_data_dict()
    def get_position(self, write_data_file = False, justOneSensor = True):
